...
```
-- These definitions are copied from the Relations module becuse they otherwise
-- depend upon the standard library definitions of things (namely natural
-- numbers).
...
+-monoˡ-≤ m n p m≤n =
  begin-≤
    (m + p)
  ≤⟨ subst (λ m+p → m+p ≤ (p + m)) (+-comm p m) ≤-refl ⟩
    (p + m)
  ≤⟨ +-monoʳ-≤ p m n m≤n ⟩
    (p + n)
  ≤⟨ subst (λ p+n → p+n ≤ (n + p)) (+-comm n p) ≤-refl ⟩
    (n + p)
  ≤-∎
